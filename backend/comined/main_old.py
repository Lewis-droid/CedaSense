...
def run_email_receiver():
    ...
    if hasattr(mod, "process_emails"):
        print("[EMAIL] Running email receiver ...")
        mod.process_emails()
    else:
...
def run_submission_extractor():
    ...
    if hasattr(mod, "process_all_existing_submissions"):
        print("[EXTRACT] Extracting text from submissions ...")
...
def append_merged_json_to_sample_input(src_json_path: str, dest_sample_input_path: str):
    ...
    try:
        ...
        print(f"[WARN] Failed to read source JSON: {e}")
        return
    ...
    try:
        ...
        print(f"[OK] Appended merged JSON into: {dest_sample_input_path}")
    except Exception as e:
